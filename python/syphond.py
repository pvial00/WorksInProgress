...
def server(s, c):
	...
	while True:
		c, addr = s.accept()
		threading.Thread(target=auth_gate, args=(c,)).start()
...
def client_handler(c, user, transit_key):
	...
	while True:
		...
		if rcmand == "":
			break
		elif rcmand[0] == "c" and rcmand [1] == "d":
			cd_args = rcmand.split(" ")
			cd_path = cd_args.pop()
			try:
				os.chdir(str(cd_path))
			except OSError as oerr:
				pass
				cd_error = "Error: path not found"
				cd_error = krypt(cd_error, transit_key)
				c.send(cd_error)
			else:
				cd_success = "\n"
				cd_success = krypt(cd_success, transit_key)
				c.send(cd_success)
			...
			try:
				output = subprocess.Popen(rcmand, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
